...
abstract final class DsTheme {
  ...
  /// Variants used when the OS asks for more contrast
  /// (`GetMaterialApp.highContrastTheme` / `highContrastDarkTheme`).
  static ThemeData lightHighContrast() => build(false, highContrast: true);
  static ThemeData darkHighContrast() => build(true, highContrast: true);

  static ThemeData build(bool isDark, {bool highContrast = false}) {
    final c = DsColors.resolve(isDark, highContrast: highContrast);
...
/// (`AppThemeData.primary300`, loaded from Firestore settings) changes, so
/// Material widgets (switches, pickers, selection handles...) always use the
/// driver brand color (`app_driver_color`).
///
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<Color>(
      ...
      builder: (context, _, _) {
        ...
        return Theme(data: DsTheme.build(isDark, highContrast: highContrast), child: widget.child);
      },
    );
  }
}
