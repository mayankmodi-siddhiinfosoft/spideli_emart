...
enum DsButtonVariant {
  ...
  /// Solid success fill. Positive, time-critical driver actions ("Accept",
  /// "Go online", "Delivered").
  success,
}
...
enum DsButtonSize {
  ...
  /// 64 – on-the-road actions (accept / reject request, start / complete
  /// trip, go online). Big enough to hit while the phone is mounted.
  xl,
}
...
/// DsButton.success(label: 'Accept'.tr, icon: Icons.check_rounded, size: DsButtonSize.xl, expand: true, onPressed: c.acceptOrder)
/// ```
class DsButton extends StatelessWidget {
  ...
  const DsButton.success({super.key, required this.label, required this.onPressed, this.size = DsButtonSize.md, this.icon, this.trailingIcon, this.leading, this.loading = false, this.expand = false, this.color, this.semanticLabel})
  ...
  Widget build(BuildContext context) {
    ...
    final textStyle = DsTypography.label
        .copyWith(
          ...
          fontWeight: size == DsButtonSize.xl ? FontWeight.w700 : FontWeight.w600,
        )
        .copyWith(color: fg);
    ...
    final radius = BorderRadius.circular(size == DsButtonSize.lg || size == DsButtonSize.xl ? DsRadius.lg : DsRadius.md);
    ...
    if ((variant == DsButtonVariant.primary || variant == DsButtonVariant.success) && !disabledLook) {
      button = DecoratedBox(
