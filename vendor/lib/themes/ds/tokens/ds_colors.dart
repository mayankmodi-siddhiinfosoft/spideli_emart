...
enum DsTone {
  ...
  /// payout, dine-in, subscription...) to a tone. Matching is keyword based
  /// and case-insensitive, so it works for `Constant.orderCompleted`,
  /// `Constant.adsRunning`, `"restaurant_rejected"`, etc.
  static DsTone fromStatus(String? status) {
    ...
    if (has(['ship', 'transit', 'driver', 'progress', 'prepar', 'pickup', 'picked'])) {
      return DsTone.info;
    }
...
/// [AppThemeData.primary300] at call time because the store brand color is
/// loaded from Firestore at runtime.
///
/// ```dart
...
class DsColors {
  ...
  /// ThemeController by GetMaterialApp.themeMode).
  static DsColors of(BuildContext context) => DsColors.resolve(Theme.of(context).brightness == Brightness.dark);

  // ---------------------------------------------------------------- brand
  /// Store brand color (runtime value of `AppThemeData.primary300`).
