...
class OnDemandPaymentController extends GetxController {
  ...
  /// Payment methods offered in the provider's region (spec 18.7).
  Future<void> _loadPaymentSettings() async {
    final order = onDemandOrderModel.value;
    final String? region = (order?.regionId?.isNotEmpty == true) ? order!.regionId : await RegionService.providerRegionId(order?.provider);
    ...
    if (!isExtra) {
      ...
      if (selectedPaymentMethod.value == PaymentGateway.wallet.name) {
        WalletTransactionModel transactionModel = WalletTransactionModel(
          ...
          regionId: onDemandOrderModel.value!.regionId,
  ...
  /// Re-filters the payment methods for [regionId] (the provider's region) when it
