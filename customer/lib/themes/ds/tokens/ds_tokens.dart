...
abstract final class DsGradients {
  /// Primary brand gradient for hero headers, balance cards, CTAs. Follows
  /// the active service section. Light accents (cab amber, parcel / rental
  /// green, on-demand cyan) are deepened so white hero text stays readable
  /// (>= 3:1 at the light end, >= 4.5:1 at the dark end).
  static LinearGradient brand(BuildContext context) {
    final c = DsColors.of(context);
    return brandFrom(c.brand);
  }

  /// [brand] built around an explicit color (e.g. `c.sectionAccent(s).main`).
  static LinearGradient brandFrom(Color b) {
    ...
    final start = DsColors.readableOn(Color.lerp(b, Colors.white, 0.06)!, Colors.white, minRatio: 3.0);
    final end = DsColors.readableOn(shifted, Colors.white, minRatio: 4.5);
    return LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [start, end]);
  }

  /// Gradient for an explicit service section.
  static LinearGradient section(BuildContext context, DsSection section) => brandFrom(DsColors.of(context).sectionAccent(section).main);
