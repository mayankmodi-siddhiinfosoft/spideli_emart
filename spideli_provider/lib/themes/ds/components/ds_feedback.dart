...
/// Current app brightness for overlays opened without a local context.
/// The provider app drives dark mode through DarkThemeProvider, which
/// rebuilds GetMaterialApp.theme, so the navigator's Theme is authoritative.
bool _isDark() {
...
///
/// ```dart
/// DsEmptyState(icon: Icons.inventory_2_outlined, title: 'No products yet'.tr, message: 'Add your first product to start selling.'.tr, actionLabel: 'Add product'.tr, onAction: ...)
/// ```
...
///
/// ```dart
/// DsInlineAlert(tone: DsTone.warning, title: 'Documents pending'.tr, message: 'Upload your documents to start receiving bookings.'.tr, actionLabel: 'Upload'.tr, onAction: ...)
/// ```
...
abstract final class DsBottomSheet {
  ...
  static Future<T?> show<T>({
    ...
    return Get.bottomSheet<T>(
      ...
      barrierColor: DsColors.resolve(_isDark()).scrim,
    );
  }
}
...
class DsDialog extends StatelessWidget {
  ...
  static Future<T?> show<T>(DsDialog dialog, {bool barrierDismissible = true}) {
    return Get.dialog<T>(dialog, barrierDismissible: barrierDismissible, barrierColor: DsColors.resolve(_isDark()).scrim, transitionDuration: DsMotion.base);
  }
  ...
  ///
  /// ```dart
  /// final ok = await DsDialog.confirm(title: 'Delete product?'.tr, message: '...'.tr, confirmLabel: 'Delete'.tr, destructive: true);
  /// if (ok) controller.deleteProduct(p);
  /// ```
