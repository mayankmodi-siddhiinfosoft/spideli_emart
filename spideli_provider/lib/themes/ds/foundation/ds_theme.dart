...
abstract final class DsTheme {
  ...
  static ThemeData build(bool isDark) {
    ...
    return ThemeData(
      ...
      brightness: brightness,
      colorScheme: scheme,
      fontFamily: DsPalette.fontFamily,
      textTheme: textTheme,
      ...
      // The provider app's shell (DashboardScreen) is drawer based.
      drawerTheme: DrawerThemeData(
        ...
        scrimColor: c.scrim,
        width: 304,
        shape: const RoundedRectangleBorder(borderRadius: BorderRadiusDirectional.horizontal(end: Radius.circular(DsRadius.xl))),
        endShape: const RoundedRectangleBorder(borderRadius: BorderRadiusDirectional.horizontal(start: Radius.circular(DsRadius.xl))),
      ),
      ...
      switchTheme: SwitchThemeData(
        thumbColor: WidgetStateProperty.resolveWith((s) => s.contains(WidgetState.selected) ? c.onBrand : (isDark ? DsPalette.grey400 : Colors.white)),
...
/// (`DsPalette.brand`, loaded from Firestore settings) changes, so
/// Material widgets (switches, pickers, selection handles...) always use the
/// provider brand color (`AppColors.colorPrimary`).
///
/// Put it in `GetMaterialApp.builder` and add [DsBrandTheme.observer] to
/// `navigatorObservers`; every route change re-checks the brand color.
class DsBrandTheme extends StatefulWidget {
  final Widget child;
  const DsBrandTheme({super.key, required this.child});

  static final ValueNotifier<Color> _brand = ValueNotifier<Color>(DsPalette.brand);

  /// Call after changing `DsPalette.brand` to re-theme immediately.
  static void refresh() {
    if (_brand.value != DsPalette.brand) _brand.value = DsPalette.brand;
  }
