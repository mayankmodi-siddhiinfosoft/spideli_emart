// Shared booking-surface pieces used by every booking screen of the Provider
// app (the five booking tabs and the booking detail), so the surfaces cannot
// drift apart.
//
...
/// Short, single-line form of a booking id: `#` plus its last 8 characters.
/// Presentation only — the full id is what gets copied.
String shortBookingId(String id) => id.length <= 8 ? '#$id' : '#${id.substring(id.length - 8)}';
...
class OrderIdHeader extends StatelessWidget {
  ...
  const OrderIdHeader({
    ...
    this.copyable = true,
    this.copiedMessage = 'Booking ID Copied',
  });
