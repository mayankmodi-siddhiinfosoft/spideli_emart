...
///
/// Implemented in-house (no `shimmer` package): a [ShaderMask] paints a
/// moving base→highlight→base gradient over the opaque pixels of [child].
/// Honors "reduce motion" (static base color) and is excluded from
/// semantics except for a single "Loading" live region.
class DsShimmer extends StatefulWidget {
  final Widget child;
  final bool enabled;

  /// Time for the highlight to sweep across once.
...
class _DsShimmerState extends State<DsShimmer> with SingleTickerProviderStateMixin {
  ...
  bool _animate(BuildContext context) => widget.enabled && !DsMotion.reduced(context);
      ...
      child: !animate
          ...
          : AnimatedBuilder(
              ...
              builder: (context, child) {
                ...
                // Sweep from -1 to 2 (in gradient widths) so the band fully
                // enters and leaves the child.
...
/// Translates a gradient horizontally by [fraction] of the painted width.
class _SlideGradient extends GradientTransform {
  final double fraction;
  const _SlideGradient(this.fraction);

  @override
  Matrix4? transform(Rect bounds, {TextDirection? textDirection}) => Matrix4.translationValues(bounds.width * fraction, 0, 0);
}
...
/// Skeleton for list screens (jobs, chats, documents...).
///
/// ```dart
...
/// Skeleton for grids (photos, documents, gallery).
