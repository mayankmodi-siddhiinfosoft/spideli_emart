...
/// (`AppColors.colorPrimary`, loaded from Firestore
/// `settings/globalSettings.worker_app_color`) changes, so Material widgets
/// (switches, pickers, selection handles...) always use the worker brand
/// color.
///
...
class DsBrandTheme extends StatefulWidget {
  ...
  static final ValueNotifier<Color> _brand = ValueNotifier<Color>(AppColors.colorPrimary);

  /// Call after changing `AppColors.colorPrimary` to re-theme immediately.
  static void refresh() {
    if (_brand.value != AppColors.colorPrimary) _brand.value = AppColors.colorPrimary;
  }
